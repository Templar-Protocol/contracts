...
3. Call `migrate` with the migration payload. For example: `migrate({"from_version":"v0","chain_id":"397"})`, then `migrate({"from_version":"v1"})` if another migration step is still required. `migrate` is private: only the universal account itself may call it, typically via a signed `FunctionCall` action.

If the account was deployed with the buggy `0.4.0` contract, the serialized state is already `V1` but the `__v` storage key is still `0`. In that case, `V1` will fail because the stored version is incorrect. Use `migrate({"from_version":"unbrick_v1"})` instead. Use `V1` only for healthy `V1 -> V2` accounts.
