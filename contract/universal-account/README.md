...
Sometimes a version upgrade will require a migration, and sometimes it will not. Freshly initialized accounts store their current state version immediately. To determine if a migration is necessary after a code upgrade, perform a view call of `needs_migration`. You can also inspect `get_stored_state_version` and `get_target_state_version`. If `needs_migration` resolves to `false`, then no migration is required. If `__v` is malformed or newer than this code supports, `needs_migration` will fail.

If a migration _is_ required, we prepare a migration payload in order to properly parameterize the migration. Multiple migrations may be required (e.g. the code was upgraded from state version 0 &rarr; 2, so it must first migrate to version 1, then to version 2). These are supplied as an ordered list and run consecutively in a single `migrate` call. The chain is validated end-to-end before any transform runs — the first step's input version must equal the stored version, each step's output version must equal the next step's input version, and the final step must land on the target version — so an invalid or incomplete chain reverts without touching state.

1. Choose the ordered list of state migrations to perform. See the [list of state migrations](https://docs.templarfi.org/doc/templar_universal_account/state/migration/enum.Migration.html). For example, upgrading from state version 0 to state version 2, we choose `V0` (0 &rarr; 1) followed by `V1` (1 &rarr; 2).
2. Parameterize each (if necessary). Some migrations require new information, some do not. In our example, the `V0` migration requires a `chain_id` parameter, so we choose `397` (NEAR Mainnet). The `V1` migration has no additional arguments.
3. Call `migrate` once with the ordered payload. For a multi-step upgrade, pass a JSON array: `migrate([{"from_version":"v0","chain_id":"397"},{"from_version":"v1"}])`. A single migration may still be passed as a bare object: `migrate({"from_version":"v1"})`. `migrate` is private: only the universal account itself may call it, typically via a signed `FunctionCall` action.

If the account was deployed with the buggy `0.4.0` contract, the serialized state is already `V1` but the `__v` storage key is still `0`. In that case, `V1` will fail because the stored version is incorrect. Use `migrate({"from_version":"unbrick_v1"})` instead — it reads the `V1` layout from stored version `0` and lands directly on `V2`. Use `V1` only for healthy `V1 -> V2` accounts.
