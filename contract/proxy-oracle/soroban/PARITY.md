...
| Freshness | `FreshnessFilter` | per-source `SourceConfig.max_age_secs` / `max_clock_drift_secs` filter before aggregation | Aggregation and breakers use the shared kernel; Soroban persists the minimum admitted-source/cache deadline |
...
| Cache invalidation | epoch bump | any policy change clears cache/history; topology/quorum change also clears breakers | Reads never recompute proof from mutable policy |
...
Verify the shared kernel and Soroban stack through the repository test
entrypoints:

```bash
just test-fast -p templar-proxy-oracle-kernel --features serde --lib
