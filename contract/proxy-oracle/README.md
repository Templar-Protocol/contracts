...
Proposals can be submitted through either of two entrypoints, which differ only in how their arguments are encoded: `create_proposal(id, operation, requested_ttl)` takes JSON and returns the stored `Proposal`, while `create_proposal_borsh` takes the same three arguments borsh-encoded and returns nothing (read the body back with `get_proposal`). Authorization, TTL resolution, create-time validation and the `Created` event are shared, so the two are interchangeable. Prefer borsh for anything carrying wasm — a `SelfUpgrade`, or an `admin_upgrade` target call — because JSON base64 inflates a blob by 4/3, and twice over for a target upgrade whose payload is itself JSON: that costs gas to parse and eats into NEAR's 1,572,864-byte `max_transaction_size`, which caps a JSON-submitted target upgrade at roughly 880 KB of wasm against borsh's ~1.17 MB. Borsh arguments are `CreateProposalArgs`, exported from `governance-common` so a client encodes the wire shape as a type rather than reproducing an argument order by hand. The borsh entrypoint is contract-side only for now: the gateway and `tmplrmgr` still submit JSON, so reaching it needs a direct contract call until the client half lands.
