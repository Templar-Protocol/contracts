...
This is intentionally a **storage oracle backed by Pyth Pro** (push → persist → serve the feed-id
...
| `templar-pyth-pro-adapter-contract` | `contract/` | NEAR cdylib: storage, governance, feed-id views. |
...
(`OracleRequest::Pyth`, by `PriceIdentifier`) — the gateway rejects that for a Pyth Pro adapter and
