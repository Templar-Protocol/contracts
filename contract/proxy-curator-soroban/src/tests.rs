...
#[test]
fn selector_wrappers_reject_ambiguous_pending_proposals_before_invoking_governance() {
    let fixture = Fixture::new();
    fixture.initialize().expect("initialize succeeds");
    let admin = Address::generate(&fixture.env);
    ...
    fixture.set_pending(PendingProposal {
        id: 61,
        action: GovernanceAction::SetCap(7, 200),
        valid_after_ns: 0,
    });

    assert_eq!(
        fixture.env.as_contract(&fixture.proxy, || {
            SorobanCuratorProxyContract::accept_cap(fixture.env.clone(), admin, 7)
        }),
        Err(ContractError::SelectorAmbiguous)
    );
    assert_eq!(
        fixture.env.as_contract(&fixture.governance, || {
            MockGovernanceContract::last_accept(fixture.env.clone())
