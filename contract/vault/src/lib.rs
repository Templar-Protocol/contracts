...
use templar_common::{
    ...
    vault::{
        ...
        EXECUTE_WITHDRAW_REQ_GAS, MAX_QUEUE_LEN, MAX_TIMELOCK_NS, MIN_TIMELOCK_NS, SUPPLY_GAS,
        WITHDRAW_GAS,
    },
};
pub use wad::*;
...
impl Contract {
    ...
    pub fn set_performance_fee(&mut self, fee: U128) {
        ...
        .emit();
        // Explicit status signal for UIs: fee enabled/disabled
        if fee_wad == wad::Wad::zero() {
            env::log_str("Performance fee disabled (rate set to 0)");
        } else {
            env::log_str("Performance fee enabled (rate > 0)");
        }
    }
    ...
    pub fn cancel(&mut self, id: U64) {
        ...
        if let Some(p) = self.pending_withdrawals.get(&id).cloned() {
            ...
            env::panic_str(&format!("Pending withdrawal not found: {}", id));
        }
    }
