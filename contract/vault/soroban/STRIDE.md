...
| **Denial of Service** | **DoS.1.R.1** — Accepted operational model: queued withdrawals require keeper/operator progression. Queue bounded by `MAX_PENDING = 1024`; `execute_withdraw` fails atomically unless the queue head is fully backed by idle assets, preventing partial queued payouts. **DoS.1.R.2** — Operate redundant keepers and queue-staleness alerts. |
