...
mod storage_tests {
    ...
    #[test]
    fn storage_codec_roundtrip_restrictions_and_rejects_bad_inputs() {
    ...
    fn storage_codec_roundtrip_state_blob_op_state_matrix() {
        ...
        let states = alloc::vec![
            OpState::Idle,
            OpState::Allocating(templar_vault_kernel::AllocatingState {
                op_id: 1,
                index: 0,
                remaining: 30,
            ...
            OpState::Refreshing(templar_vault_kernel::RefreshingState {
                op_id: 4,
                index: 1,
                plan: alloc::vec![3, 4, 5],
            }),
            OpState::Payout(templar_vault_kernel::PayoutState {
                op_id: 5,
                request_id: 6,
        ...
        for (index, op_state) in states.into_iter().enumerate() {
            let versioned = VersionedState::new(VaultState {
                total_assets: 1_000 + index as u128,
                total_shares: 2_000 + index as u128,
                idle_assets: 300 + index as u128,
                external_assets: 700 + index as u128,
                fee_anchor: FeeAccrualAnchor::new(
                    900 + index as u128,
                    TimestampNs(1_000 + index as u64),
                ),
                op_state,
                withdraw_queue: withdraw_queue.clone(),
                next_op_id: 10 + index as u64,
            });

            let encoded = fuzz_api::encode_state_blob_bytes(&versioned);
            let decoded =
                fuzz_api::decode_state_blob_bytes(&encoded).expect("state matrix roundtrip");
            assert_eq!(decoded, versioned);
        }
    }
