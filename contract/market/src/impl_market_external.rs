...
impl MarketExternalInterface for Contract {
    ...
    fn get_last_yield_rate(&self) -> Decimal {
        let deposited: Decimal = self.current_snapshot.get_deposited_active().into();
        if deposited.is_zero() {
            return Decimal::ZERO;
        }
        let borrowed: Decimal = self.current_snapshot.get_borrowed().into();
        ...
        self.current_snapshot.get_interest_rate() * borrowed * supply_weight
            / deposited
            / total_weight
    }
