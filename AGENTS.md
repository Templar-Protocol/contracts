...
- **Releasing a new version of an existing contract**: nothing to do by hand. Merging the Release PR tags it; CI builds the WASM reproducibly at that tag, uploads it, and adds one file under `contract/artifacts/releases/` on the catalog PR the whole release batch shares. See `RELEASING.md`.
