...
| Proxy oracle governance `proxy-gov-<market>` | Timelocked owner of a proxy oracle; every change is a maturing proposal, except immediate risk-reducing trips. | Admin role held by the DAO multisig. | [Governance](./governance.md#proxy-oracle-governance) |
...
| HOT Labs omni bridge (`v2_1.omni.hot.tg`) | Representation of Stellar-origin assets on NEAR. | Cannot act on Templar contracts. | **Input needed**: audit references for the HOT omni bridge. |
...
**Input needed**: the DeFiSafety review references a `Templar-Protocol/architecture` repository; confirm whether it is public and current so it can be linked here.
