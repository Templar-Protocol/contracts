...
CREATE INDEX IF NOT EXISTS gateway_operations_status_updated_at_idx ON gateway_operations (STATUS, updated_at);
...
ALTER TABLE
    gateway_operations DROP CONSTRAINT IF EXISTS gateway_operations_request_fingerprint_hash_length_check;

ALTER TABLE
    gateway_operations
ADD
    CONSTRAINT gateway_operations_request_fingerprint_hash_length_check CHECK (octet_length(request_fingerprint_hash) = 32);
...
ALTER TABLE
    gateway_operation_steps DROP CONSTRAINT IF EXISTS gateway_operation_steps_state_payload_check;

ALTER TABLE
    gateway_operation_steps
ADD
    CONSTRAINT gateway_operation_steps_state_payload_check CHECK (
        (
            state = 'not_started'
            AND tx_hash IS NULL
...
CREATE INDEX IF NOT EXISTS gateway_operation_steps_operation_id_step_index_idx ON gateway_operation_steps (operation_id, step_index);
