...
impl DispatchRead<GatewayContext> for token::GetBalanceOf {
    fn dispatch(
        request: Self::Input,
        ctx: GatewayContext,
...
impl PlanWrite<GatewayContext> for token::Transfer {
    fn plan(
        request: Self::Input,
        ctx: GatewayContext,
...
impl PlanWrite<GatewayContext> for token::TransferCall {
    fn plan(
        request: Self::Input,
        ctx: GatewayContext,
    ) -> BoxFuture<'static, GatewayResult<OperationPlan>> {
