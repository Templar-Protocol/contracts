...
use templar_manager::commands::{
    ...
    redstone_adapter::{
        ...
        deploy::{DeployRedStoneAdapter, RedStoneAdapterInitArgs, RedStoneArgsLoader},
        feed::get::FeedGet,
...
fn test_config_source() -> RedStoneArgsLoader {
    RedStoneArgsLoader::test()
}
...
fn prod_config_with_relaxed_timestamps() -> RedStoneArgsLoader {
    ...
    RedStoneArgsLoader::from_json_string(
        serde_json::to_string(&RedStoneAdapterInitArgs { config }).unwrap(),
    )
}
...
async fn redstone_adapter_deploy_from_configuration_file(#[future(awt)] worker: Worker<Sandbox>) {
    ...
    let configuration_file = write_json_file(
        "redstone-configuration",
        &RedStoneAdapterInitArgs {
            config: config.clone(),
        },
    );
    ...
    DeployRedStoneAdapter {
        deploy: Deploy::native(
            signer_args(&adapter),
            no_build_loader(),
            RedStoneArgsLoader::from_file(configuration_file.clone()),
        ),
    }
    .run(&ctx)
    .await
...
async fn redstone_adapter_write_prices(#[future(awt)] worker: Worker<Sandbox>) {
    ...
    DeployRedStoneAdapter {
        deploy: Deploy::native(
            signer_args(&adapter),
            no_build_loader(),
            prod_config_with_relaxed_timestamps(),
        ),
    }
    .run(&ctx)
    .await
