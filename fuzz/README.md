...
| `fuzz_soroban_storage_codecs` | Real encode→decode round-trips for every policy/state codec, including the production V2 paged state format (header + withdraw-queue pages → `compose_state_from_header`). (Raw-byte decoding isn't fuzzed — it over-allocates on an unbounded length prefix, [ENG-345](https://linear.app/templar-protocol/issue/ENG-345).) |
| `fuzz_fee_math` | Failable properties of the real fee-accrual / share-conversion math: the accrual basis never exceeds current assets, fee-share calcs are zero in their degenerate cases, and each `convert_to_*_bounded` matches its unbounded sibling within the cap (differential oracle). |
