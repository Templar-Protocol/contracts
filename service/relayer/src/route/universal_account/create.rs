...
pub async fn create(
        ...
        .await
    {
        ...
        // A database/lock failure happens before anything is submitted, so it is
        // not a deploy failure; keep the two distinguishable to the caller.
        Err(e @ (SubmitError::Database(_) | SubmitError::Lock(_))) => {
