    ...
    ChunkedAppendOnlyList<T, CHUNK_SIZE>
{
    ...
    pub fn push(&mut self, item: T) {
        if self.last_chunk_next_index == 0 {
            ...
            let last_inner = self
                ...
                .len()
                .checked_sub(1)
                .unwrap_or_else(|| crate::panic_with_message("Inconsistent state: len == 0"));
            let v = self
                .inner
                .get_mut(last_inner)
                .unwrap_or_else(|| crate::panic_with_message("Inconsistent state: tail dne"));
            v.push(item);
        }
    ...
    pub fn replace_last(&mut self, item: T) {
        let Some(entry) = self
            ...
            crate::panic_with_message("Cannot replace_last in empty list");
        };
        *entry = item;
    }
