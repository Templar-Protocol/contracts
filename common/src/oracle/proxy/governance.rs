...
use crate::{oracle::pyth::PriceIdentifier, time::Nanoseconds};
...
impl<T: Clone + Serialize + BorshSerialize + BorshDeserialize> Governance<T> {
    ...
    pub fn execute(&mut self, id: u32, now: Nanoseconds) -> Result<T, error::ExecuteError> {
        if id >= self.next_id {
            return Err(error::IdOutOfBoundsError {
                exclusive_maximum: self.next_id,
                actual: id,
...
mod tests {
    ...
    fn create() {
        ...
        assert_eq!(g.execute(0, now).unwrap(), "hello");
        ...
        assert_eq!(g.execute(1, now).unwrap(), "hello 4");
